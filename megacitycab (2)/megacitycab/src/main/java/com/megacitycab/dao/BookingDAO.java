...
public class BookingDAO {
    ...
    private static final String DB_URL = "jdbc:mysql://localhost:3306/poornacab?deregisterDriversOnClose=true";
...
//    public void AssignDriver(int Drive_id,int bookingId )throws SQLException{
//      
//         try (Connection connection = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD)) {
//            String query = "UPDATE `bookings` SET driver_id`=? WHERE `booking_id`=?";
//            try (PreparedStatement statement = connection.prepareStatement(query)) {
//                statement.setInt(1, Drive_id);
...
//    }
    ...
    public String AssignDriver(int Drive_id, int bookingId) throws SQLException {
    try (Connection connection = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD)) {
        String query = "UPDATE `bookings` SET `driver_id`=? WHERE `booking_id`=?";
        try (PreparedStatement statement = connection.prepareStatement(query)) {
            statement.setInt(1, Drive_id);
